...
Output:
    * siphonophores.bib in repo root — but see below.

.. warning::

    This generation is **historical**. ``siphonophores.bib`` is now the source
    of truth and is edited directly, so re-running this script would discard
    every correction made since it was first built. It refuses to overwrite an
    existing ``siphonophores.bib`` without ``--force``; pass ``--out`` to
    regenerate elsewhere for comparison. The docx is retained for provenance
    only, and the four ``build/`` inputs named above are no longer kept, so a
    from-scratch rebuild means re-running the whole parse/match/DOI/Crossref
    chain first.
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help="Overwrite siphonophores.bib even though it is the source of "
             "truth. Discards every edit made since the last generation.",
    )
    ap.add_argument(
        "--out", type=Path, default=OUT_BIB, metavar="PATH",
        help=f"Write somewhere other than {OUT_BIB.name} (default: {OUT_BIB.name}). "
             "Use this to regenerate for comparison without touching the live bib.",
    )
    args = ap.parse_args()

    out_bib = args.out
    # siphonophores.bib is hand-edited and authoritative; regenerating it
    # silently discards every keeppages/doclang/license/note correction made
    # since. The original docx generation is finished and its inputs
    # (entries.json, match.json, dois.json, crossref_dois.json) are no longer
    # kept under build/, so an accidental run would usually fail messily
    # anyway. Refuse by default and make the operator say so explicitly.
    if out_bib.exists() and not args.force:
        raise SystemExit(
            f"{out_bib} already exists and is the source of truth for this "
            "library — regenerating it would discard every edit made since it "
            "was first built.\n"
            "  To regenerate for comparison:  python scripts/build_bib.py "
            "--out build/siphonophores.regenerated.bib\n"
            "  To overwrite anyway:           python scripts/build_bib.py --force"
        )
